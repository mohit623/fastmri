...
def build_args():
    ...
    default_root_dir = fetch_dir("log_path", path_config) / "r2unet"
    ...
    parser.set_defaults(
        in_chans=1,  # number of input channels to R2U-Net
        out_chans=1,  # number of output chanenls to R2U-Net
        chans=32,  # number of top-level R2U-Net channels
        num_pool_layers=4,  # number of R2U-Net pooling layers
    ...
    #-------------------------------------------------------------------------
    # This script runs in either train or test mode. In test mode, we are
    # using the saved model weights via the checkpoints.
    # So disable checkpointing.
    #-------------------------------------------------------------------------
    if args.mode == 'train':
